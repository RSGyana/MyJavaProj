...
public class OrderMethodUtil {
	...
	public void generatePie(String path,List<Object[]> data) {
		...
		JFreeChart chart=ChartFactory.createPieChart3D("OrderMethod Modes", dataset, true, true, false);
		//3. save as Image using ChartUtils
		try {
		ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/ordermethodA.jpg"), chart, 200, 200);
		} catch (IOException e) {
		e.printStackTrace();
		}
	}
		...
		//2. Create JFreeChart using ChartFactory
		JFreeChart chart=ChartFactory.createBarChart("OrderMethod Mode", "Modes", "COUNT", dataset);
		//3. Save as Image using ChartUtil
		try {
		ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/ordermethodB.jpg"), chart, 200, 200);
		} catch (IOException e) {
		e.printStackTrace();
		}
		}
